...
class CredentialChecker(object):
    async def is_payload(self, payload: Union[dict[str, Any], None]) -> bool:
        return payload is not None

    async def is_bearer(self, credentials: Union[HTTPAuthorizationCredentials, None]) -> bool:
...
class JWTBearerInactive(HTTPBearer, CredentialChecker):
    ...
    async def __call__(self, request: Request) -> HTTPAuthorizationCredentials:
        credentials = await super(JWTBearerInactive, self).__call__(request)
        if credentials:
            if not await self.is_bearer(credentials):
                raise HTTPException(status_code=403, detail="Invalid authentication scheme.")
            payload = await self.verify_jwt(credentials.credentials)
...
class JWTBearerAdmin(HTTPBearer, CredentialChecker):
    ...
    async def __call__(self, request: Request) -> HTTPAuthorizationCredentials:
        credentials = await super(JWTBearerAdmin, self).__call__(request)
        if credentials:
            if not await self.is_bearer(credentials):
                raise HTTPException(status_code=403, detail="Invalid token or expired token.")
            payload = await self.verify_jwt(credentials.credentials)
